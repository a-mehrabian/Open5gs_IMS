...
# CA self certificate
openssl req  -new -batch -x509 -days 3650 -nodes -newkey rsa:1024 -out $1/cacert.pem -keyout cakey.pem -subj /CN=ca.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=CHRONOS/OU=Tests

#mme
openssl genrsa -out $1/mme.key.pem 1024
openssl req -new -batch -out mme.csr.pem -key $1/mme.key.pem -subj /CN=mme.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=CHRONOS/OU=Tests
...
openssl req -new -batch -out hss.csr.pem -key $1/hss.key.pem -subj /CN=hss.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=CHRONOS/OU=Tests
...
openssl req -new -batch -out smf.csr.pem -key $1/smf.key.pem -subj /CN=smf.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=CHRONOS/OU=Tests
...
openssl req -new -batch -out pcrf.csr.pem -key $1/pcrf.key.pem -subj /CN=pcrf.EPC_DOMAIN/C=KO/ST=Seoul/L=Nowon/O=CHRONOS/OU=Tests
