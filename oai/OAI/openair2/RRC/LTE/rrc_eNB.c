...
//-----------------------------------------------------------------------------
{
  ...
  if (measResults2->measId > 0 ) {    
    for (int i = 0; i < MAX_LOCALIZATION_UE; i++) {
      if (RC.HL[i].rnti == ctxt_pP->rntiMaybeUEid) {
        UE_id_HL = i;
        break;
      }
    }
    ...
    if (UE_id_HL != -1) RC.HL[UE_id_HL].RRC.rsrp_serving_cell = ue_context_pP->ue_context.measResults->measResultPCell.rsrpResult - 140;
    if (UE_id_HL != -1) RC.HL[UE_id_HL].RRC.rsrq_serving_cell = ue_context_pP->ue_context.measResults->measResultPCell.rsrqResult / 2 - 20;
    LOG_D(RRC,
  ...
  if (measResults2->measResultNeighCells->choice.measResultListEUTRA.list.count > 0) {
    ...
    for (i=0; i < neighboring_cells; i++) {
      ...
      if (i < MAXIMUM_NEIGHBOR_CELL_L) {
        if (UE_id_HL != -1) RC.HL[UE_id_HL].RRC.rsrp_neighbor_cell[i] = (*ue_context_pP->ue_context.measResults->measResultNeighCells->choice.measResultListEUTRA.list.array[i]->measResult.rsrpResult)-140;
        if (UE_id_HL != -1) RC.HL[UE_id_HL].RRC.rsrq_neighbor_cell[i] = (*ue_context_pP->ue_context.measResults->measResultNeighCells->choice.measResultListEUTRA.list.array[i]->measResult.rsrqResult)/2 - 20;
      }
