...
//-----------------------------------------------------------------------------
{
  ...
  if (UE_id != -1) {
    ...
    //if (UE_id_HL != -1) RC.HL[UE_id].rnti = current_rnti;
    if (ul_cqi && UE_id_HL != -1) {
      RC.HL[UE_id_HL].MAC.ULSDU.ul_cqi = ul_cqi;
    }
    ...
    if (timing_advance && UE_id_HL != -1) {
      RC.HL[UE_id_HL].MAC.ULSDU.timing_advance = timing_advance;
    }
    ...
    if (sduP != NULL) {
      ...
      if(tmp_snr > 0 && tmp_snr < 63) {
        ...
        if (UE_scheduling_control->pusch_snr[CC_idP] && UE_id_HL != -1) {
          RC.HL[UE_id_HL].MAC.ULSDU.pusch_snr[CC_idP] = UE_scheduling_control->pusch_snr[CC_idP];
          RC.HL[UE_id_HL].MAC.ULSDU.pusch_snr_avg[CC_idP] = UE_scheduling_control->pusch_snr_avg[CC_idP];
        }
      }
  ...
  for (int i = 0; i < num_ce; i++) {
    ...
    if (est_ue_power) {
      RC.HL[UE_id_HL].MAC.ULSDU.ue_power_headroom = UE_template_ptr->phr_info;
      RC.HL[UE_id_HL].MAC.ULSDU.estimated_ue_power = est_ue_power;
    }
