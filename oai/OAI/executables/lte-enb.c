...
void dumpLocalizationStats(FILE *fd, Localization_t *HL, int HLLength) {
    ...
    for (int i = 0; i < HLLength; i++) {
        if (HL[i].rnti != 0) {
            ...
            pos_rssi = 0;
            //char out_rssi[1000];
            pos_rssi += sprintf(out_rssi + pos_rssi, "\"rx_spatial_power_db\": [[%u,%u,%u,%u]", HL[i].L1.L1Measurments.rx_spatial_power_dB[0][0][0], HL[i].L1.L1Measurments.rx_spatial_power_dB[0][0][1], HL[i].L1.L1Measurments.rx_spatial_power_dB[0][1][0], HL[i].L1.L1Measurments.rx_spatial_power_dB[0][1][1]);
            for (int i = 1; i < NUMBER_OF_SRS_MAX_L; i++) {
              pos_rssi += sprintf(out_rssi + pos_rssi, ",[%u,%u,%u,%u]", HL[i].L1.L1Measurments.rx_spatial_power_dB[i][0][0], HL[i].L1.L1Measurments.rx_spatial_power_dB[i][0][1], HL[i].L1.L1Measurments.rx_spatial_power_dB[i][1][0], HL[i].L1.L1Measurments.rx_spatial_power_dB[i][1][1]);
            }
            pos_rssi += sprintf(out_rssi + pos_rssi, "]");
            pos += sprintf(output + pos, "%s", out_rssi);
            ...
            pos += sprintf(output + pos, "\"SRS\": {");
            pos += sprintf(output + pos, "\"channel_estimate_per_ofdm\": [%d, %d, %d, %d],", HL[i].L1.SRS.srs_ch_estimates[0][0], HL[i].L1.SRS.srs_ch_estimates[0][2], 0, 0);
            pos += sprintf(output + pos, "\"channel_estimate_time_per_ofdm\": [%d, %d, %d, %d]", HL[i].L1.SRS.srs_ch_estimates_time[0][0], HL[i].L1.SRS.srs_ch_estimates_time[0][2], 0, 0);
            //pos += sprintf(output + pos, "\"srs\": [%d, %d, %d, %d]", HL[i].L1.SRS.srs[0], 0, 0, 0);
            pos += sprintf(output + pos, "}"); // end of UCI
...
void *localization_stats_thread(void *param) {
  ...
  while (!oai_exit) {
    ...
    for (int ii = 0; ii < MAX_LOCALIZATION_UE; ii++) {
      if ((&eNB->soundingrs_ul_config_dedicated[ii])->rnti == RC.HL[ii].rnti) {
        UE_id = ii;
        break;
      }
    }
    if (UE_id != -1) {
      if (eNB->measurements.n0_power_tot) RC.HL[UE_id].L1.L1Measurments.n0_power_tot = eNB->measurements.n0_power_tot;
      if (eNB->measurements.n0_power_tot_dB) RC.HL[UE_id].L1.L1Measurments.n0_power_tot_dB = eNB->measurements.n0_power_tot_dB;
      if (eNB->measurements.n0_subband_power_avg_dB) RC.HL[UE_id].L1.L1Measurments.n0_subband_power_avg_dB = eNB->measurements.n0_subband_power_avg_dB;
      for (int i = 0; i < NUMBER_OF_SRS_MAX_L; i++) {
        if (eNB->measurements.rx_rssi_dBm[i]) RC.HL[UE_id].L1.L1Measurments.rx_rssi_dBm[i] = eNB->measurements.rx_rssi_dBm[i];
        for (int j = 0; j < 2; j++)  {
          for (int k = 0; k < 2; k++) {
            if (eNB->measurements.rx_spatial_power[i][j][k]) RC.HL[UE_id].L1.L1Measurments.rx_spatial_power[i][j][k] = eNB->measurements.rx_spatial_power[i][j][k];
            if (eNB->measurements.rx_spatial_power_dB[i][j][k]) RC.HL[UE_id].L1.L1Measurments.rx_spatial_power_dB[i][j][k] = eNB->measurements.rx_spatial_power_dB[i][j][k];
          }
        }
      }
    }
