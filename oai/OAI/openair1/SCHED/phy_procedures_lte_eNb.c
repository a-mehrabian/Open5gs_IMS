        ...
        if (eNB->soundingrs_ul_config_dedicated[i].active == 1 &&
            ...
            eNB->soundingrs_ul_config_dedicated[i].subframe == subframe) {
              if (lte_srs_channel_estimation (fp, &eNB->common_vars, &eNB->srs_vars[i], &eNB->soundingrs_ul_config_dedicated[i], subframe, 0 /*eNB_id */ )) {
                ...
                lte_eNB_srs_measurements(eNB, 0, i, 0);
                int UE_id = -1;
                for (int ii = 0; ii < MAX_LOCALIZATION_UE; ii++) {
                  if ((&eNB->soundingrs_ul_config_dedicated[i])->rnti == RC.HL[ii].rnti) {
                    UE_id = ii;
                    break;
                  }
                }
                if (UE_id != -1) {
                  for (int aa = 0; aa < MAXIMUM_ANNTENA_L; aa++) {
                    RC.HL[UE_id].L1.SRS.srs[0] = (&eNB->srs_vars[i])->srs[0];
                    for (int j = 0; j < MAXIMUM_OFDM_SYMBOL_SIZE_L; j++) {
                      RC.HL[UE_id].L1.SRS.srs_ch_estimates[aa][j] = (&eNB->srs_vars[i])->srs_ch_estimates[aa][j];
                      RC.HL[UE_id].L1.SRS.srs_ch_estimates_time[aa][j] = (&eNB->srs_vars[i])->srs_ch_estimates_time[aa][j];
                    }
                  }
                }
              }              
